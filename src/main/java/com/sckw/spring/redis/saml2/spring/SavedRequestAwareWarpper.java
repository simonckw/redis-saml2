...
/**
 * Provides request parameters, headers and cookies from either an original request or a saved
 * request.
 *
 * <p>
...
class SavedRequestAwareWrapper extends HttpServletRequestWrapper {
   ...
   * SimpleDateFormat is not thread-safe, we can't declare formats[] as a static variable.
   */
   ...
   * forwarded/included to a URL with parameters, either supplementing or overriding the saved
   * request values.
   * <p>
   * In this case, the value from the wrapped request should be used.
   * <p>
   * If the value from the wrapped request is null, an attempt will be made to retrieve the
   * parameter from the saved request.
   */
  @Override
